...
require 'generators/i18n_proofreading/install/install_generator'
...
class BackboneTest < ActionDispatch::IntegrationTest
  # 1. A host that replaces the layout must still get the dashboard's stylesheet.
  #    It used to be declared in the gem's own layout, so `admin_layout` (and now
  #    base_controller_class) silently dropped it: unstyled dashboard.
  test 'every dashboard page carries its own assets, under any layout' do
    allow_admin!
    suggestion = I18nProofreading::Suggestion.create!(
      translation_key: 'sample.greeting', locale: 'en', proposed_value: 'Hi'
    )

    [nil, 'host_admin'].each do |layout|
      I18nProofreading.config.admin_layout = layout if layout
      ['/i18n_proofreading/', "/i18n_proofreading/suggestions/#{suggestion.id}"].each do |path|
        get path
        ...
        assert_includes response.body, 'class="ip-dashboard"', "#{path} is missing the scoping wrapper"
      end
    end
  end
  ...
  test 'the stylesheet claims no selector outside its own namespace' do
    ...
    top_level_selectors(response.body).each do |part|
      assert part.start_with?('.ip-', ':root'),
  ...
  test 'the stylesheet prefixes every custom property' do
    ...
    properties.each do |property|
      assert property.start_with?('--ip-'), "custom property #{property} could collide with a host's"
    end
  end
  ...
  #    controller would demand a staff session from every proofreader.
  test 'only the dashboard hangs off the configured base controller' do
    assert_equal I18nProofreading.base_controller, I18nProofreading::DashboardController.superclass
    assert_equal ActionController::Base, I18nProofreading::ApplicationController.superclass

    [I18nProofreading::WidgetsController, I18nProofreading::SubmissionsController].each do |controller|
      assert_equal I18nProofreading::ApplicationController, controller.superclass,
                   "#{controller} must not inherit the host's base controller"
    end
    assert_equal I18nProofreading::DashboardController, I18nProofreading::SuggestionsController.superclass
  end

  test 'base_controller_class resolves a host class lazily, by name' do
    assert_equal 'ActionController::Base', I18nProofreading.config.base_controller_class
    I18nProofreading.config.base_controller_class = 'HostAdminBaseController'

    assert_equal HostAdminBaseController, I18nProofreading.base_controller
  end

  # 5. The widget reads context and posts suggestions. Both have to keep working
  #    no matter what the dashboard inherits, which is why they route to a
  #    different controller — at the same URLs as before.
  test 'the widget endpoints split away from the dashboard' do
    engine = I18nProofreading::Engine.routes

    assert_equal 'i18n_proofreading/submissions',
                 engine.recognize_path('/suggestions', method: :post)[:controller]
    assert_equal 'i18n_proofreading/submissions',
                 engine.recognize_path('/suggestions/context', method: :get)[:controller]
    assert_equal 'i18n_proofreading/suggestions',
                 engine.recognize_path('/suggestions', method: :get)[:controller]
    assert_equal 'i18n_proofreading/widgets',
                 engine.recognize_path('/dashboard.css', method: :get)[:controller]
  end

  test 'record routes accept a non-integer primary key' do
    uuid = '0f8fad5b-d9cb-469f-a165-70867728950e'

    assert_equal({ controller: 'i18n_proofreading/suggestions', action: 'show', id: uuid },
                 I18nProofreading::Engine.routes.recognize_path("/suggestions/#{uuid}", method: :get))
  end
  ...
  test 'the page frame is declared after the component layer' do
    ...
    components = response.body.index('.ip-dashboard {')
...
class BackboneGeneratorTest < Rails::Generators::TestCase
  tests I18nProofreading::Generators::InstallGenerator
  ...
  test 'the table follows the host generators primary_key_type' do
    with_primary_key_type(:uuid) do
      ...
      assert_migration 'db/migrate/create_i18n_proofreading_suggestions.rb' do |migration|
        assert_match 'create_table :i18n_proofreading_suggestions, id: :uuid', migration
  ...
  test 'the table takes no id option when the host sets nothing' do
    with_primary_key_type(nil) do
      ...
      assert_migration 'db/migrate/create_i18n_proofreading_suggestions.rb' do |migration|
        assert_match 'create_table :i18n_proofreading_suggestions do |t|', migration
      end
    end
  end
